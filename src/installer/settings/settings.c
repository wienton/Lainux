...
const char *get_keyboard_name(void) {
  return (settings.keyboard_layout == 0) ? get_text("KB_EN")
                                         : get_text("KB_RU");
}

const char *get_network_mode_name(void) {
  return (settings.network_mode == 0) ? get_text("NET_DHCP")
                                      : get_text("NET_STATIC");
}
...
void print_settings(void) {
  ...
  while (!quit) {
    ...
    // pointers menu
    const char *items[] = {get_text("LANG_SETTING"), get_text("THEME_SETTING"),
    ...
    for (int i = 0; i < total_items; i++) {
      ...
      switch (i) {
      case 0:
        value = (settings.language == LANG_RU) ? "Русский" : "English";
        break;
      case 1:
        value = get_theme_name();
        break;
      case 2:
        value = get_keyboard_name();
        break;
      case 3:
        value = get_network_mode_name();
        break;
      default:
        continue;
      }
